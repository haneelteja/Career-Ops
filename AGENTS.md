...
- Submit only when `config/profile.yml` → `application.auto_submit_after_verification` is `true` and the verification gate in `modes/apply.md` is complete; otherwise stop before Submit/Apply.
